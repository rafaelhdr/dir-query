...
The website-widget capability lets a workspace's chat panel be embedded
on any third-party site via a single `<script>` tag (`widget.js`), so a
visitor to that site can ask questions of the workspace without
visiting Dir Query directly. The script supports two embed modes —
a floating launcher bubble ("widget") or direct in-page mounting
("inline") — and otherwise behaves the same as the workspace's own ask
page: fetching workspace data, submitting questions, streaming
answers, and storing the resulting conversations.
