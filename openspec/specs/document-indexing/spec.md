...
The document-indexing capability turns uploaded PDFs into a retrievable
form, scoped per workspace. Indexing happens in the background after
upload, uses a local embedding model (no external API required), persists
chunks and embeddings as rows in Postgres (`chunks`, with a `pgvector`
`embedding` column) durably across restarts, and incrementally syncs on
every backend startup — driven by the `files.status` column — so only
pending files are (re-)indexed.
