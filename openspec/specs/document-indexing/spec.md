...
The document-indexing capability turns uploaded PDFs into a retrievable form. Indexing happens in the background after upload, uses a local embedding model (no external API required), persists to disk durably across restarts, and incrementally syncs on every backend startup so only new files are (re-)indexed.
